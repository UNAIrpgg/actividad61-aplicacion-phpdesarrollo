...
//Se comprueba si existen campos del formulario vacíos
	if(empty($name) || empty($surname) || empty($age) || empty($job))	{
		...
		if(empty($job)) {
			echo "<font color='red'>Campo de la rehabilitacion vacío.</font><br/>";
		}
		  if(empty($date)) {
                        echo "<font color='red'>Campo fecha vacío.</font><br/>";
                }
	...
	else //Se realiza la modificación de un registro de la BD. 
	{
		...
		$mysqli->query("UPDATE empleados SET nombre = '$name', apellido = '$surname',  edad = '$age', estado = '$job', fecha = '$date' WHERE id = $idempleado");
		$mysqli->close();
